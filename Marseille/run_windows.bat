@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\MRS
set VERSION=1.30.0
echo [Marseille Mod] Copying data files to cities\data\MRS...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\MRS\*" "%TARGET%\" >nul
echo [Marseille Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Marseille Mod] Downloading pmtiles.exe...
...
echo [Marseille Mod] Starting tile server on port 8080...
