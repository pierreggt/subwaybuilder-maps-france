...
TARGET="$SCRIPT_DIR/../../cities/data/CFE"
VERSION="1.30.0"
echo "[Clermont-Ferrand Mod] Copying data files to cities/data/CFE..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/CFE/"* "$TARGET/"
echo "[Clermont-Ferrand Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Clermont-Ferrand Mod] Downloading pmtiles..."
...
echo "[Clermont-Ferrand Mod] Starting tile server on port 8090..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8090 --cors=*
