...
TARGET="$SCRIPT_DIR/../../cities/data/MNC"
VERSION="1.30.0"
echo "[Metz-Nancy Mod] Copying data files to cities/data/MNC..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/MNC/"* "$TARGET/"
echo "[Metz-Nancy Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Metz-Nancy Mod] Downloading pmtiles..."
...
echo "[Metz-Nancy Mod] Starting tile server on port 8104..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8104 --cors=*
