...
TARGET="$SCRIPT_DIR/../../cities/data/COR"
VERSION="1.30.0"
echo "[Corse Mod] Copying data files to cities/data/COR..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/COR/"* "$TARGET/"
echo "[Corse Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Corse Mod] Downloading pmtiles..."
...
echo "[Corse Mod] Starting tile server on port 8103..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8103 --cors=*
