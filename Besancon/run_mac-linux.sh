...
TARGET="$SCRIPT_DIR/../../cities/data/BES"
VERSION="1.30.0"
echo "[Besancon Mod] Copying data files to cities/data/BES..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/BES/"* "$TARGET/"
echo "[Besancon Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Besancon Mod] Downloading pmtiles..."
...
echo "[Besancon Mod] Starting tile server on port 8085..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8085 --cors=*
