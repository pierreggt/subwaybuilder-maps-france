...
TARGET="$SCRIPT_DIR/../../cities/data/TOU"
VERSION="1.30.0"
echo "[Tours Mod] Copying data files to cities/data/TOU..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/TOU/"* "$TARGET/"
echo "[Tours Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Tours Mod] Downloading pmtiles..."
...
echo "[Tours Mod] Starting tile server on port 8082..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8082 --cors=*
