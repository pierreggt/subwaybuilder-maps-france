...
TARGET="$SCRIPT_DIR/../../cities/data/CDK"
VERSION="1.30.0"
echo "[Calais - Dunkerque Mod] Copying data files to cities/data/CDK..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/CDK/"* "$TARGET/"
echo "[Calais - Dunkerque Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Calais - Dunkerque Mod] Downloading pmtiles..."
...
echo "[Calais - Dunkerque Mod] Starting tile server on port 8098..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8098 --cors=*
