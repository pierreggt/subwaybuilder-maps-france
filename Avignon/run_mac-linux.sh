...
TARGET="$SCRIPT_DIR/../../cities/data/AVI"
VERSION="1.30.0"
echo "[Avignon Mod] Copying data files to cities/data/AVI..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/AVI/"* "$TARGET/"
echo "[Avignon Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Avignon Mod] Downloading pmtiles..."
...
echo "[Avignon Mod] Starting tile server on port 8092..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8092 --cors=*
