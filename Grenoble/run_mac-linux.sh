...
TARGET="$SCRIPT_DIR/../../cities/data/GRE"
VERSION="1.30.0"
echo "[Grenoble Mod] Copying data files to cities/data/GRE..."
mkdir -p "$TARGET"
cp -f "$SCRIPT_DIR/data/GRE/"* "$TARGET/"
echo "[Grenoble Mod] Data files copied successfully."
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    echo "[Grenoble Mod] Downloading pmtiles..."
...
echo "[Grenoble Mod] Starting tile server on port 8089..."
"$SCRIPT_DIR/pmtiles" serve "$SCRIPT_DIR" --port 8089 --cors=*
