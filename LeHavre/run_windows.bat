@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\LHA
set VERSION=1.30.0
echo [Le Havre Mod] Copying data files to cities\data\LHA...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\LHA\*" "%TARGET%\" >nul
echo [Le Havre Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Le Havre Mod] Downloading pmtiles.exe...
...
echo [Le Havre Mod] Starting tile server on port 8080...
